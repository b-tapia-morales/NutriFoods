...
public class IngredientRecipe
{
    ...
    private void InsertIngredientRecipe(string pathReceta)
    {
        ...
        var path = Path.Combine(directory, "RecipeAndMesuris", "Recipe_insert", "Recipe", "gourmet",
            $"recetas_{pathReceta}.txt");
        StreamReader fileRecipe = new StreamReader(path);

        while (!fileRecipe.EndOfStream)
        {
            String line = fileRecipe.ReadLine() ?? throw new InvalidOperationException();
            String[] spilt = line.Split(";");
            var ingredientPath = Path.Combine(directory, "RecipeAndMesuris", "Recipe_insert", "Ingredient",
                "parcerIngredientes", "Ingredientes_Gourmet", pathReceta, $"ingre_{spilt[0]}.txt");
            ReadFileRecipeIngredient(ingredientPath, spilt[0]);
        }
    ...
    private void TransformUnits(String quantity, String units, String ingredient, String pathReceta, String recipeId)
    {
        ...
        if (ingredient.Equals("Pimiento Ahumano") || ingredient.Equals("Pimento Ahumado") ||
            ingredient.Equals("Pimentón Ahumado")) ingredient = "Paprika";
        if (ingredient.Equals("pechugas")) ingredient = "Pechuga de pollo";
        if (ingredient.Equals("pasta ají amarillo")) ingredient = "Pasta de aji";
        if (ingredient.Equals("yoghurt natural") || ingredient.ToLower().Equals("yogurts naturales"))
            ingredient = "Yogurt";
        ...
        if (ingredientResult != null)
        {
            if (units.Equals("x") || units.Equals("unidad") || units.Equals("unidas")) 
            {
                ...
                if (listGram.Count > 0)
                {
                    if (!ingredient.Equals("Caldo de Verduras"))
                    {
                        ...
                        string insertRecipeQuantity =
                            $"INSERT INTO nutrifoods.recipe_quantity (recipe_id, ingredient_id, grams) " +
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ((ingredient.Equals("vino blanco") || ingredient.Equals("vino tinto")) &&
                        units.Replace("s", "").ToLower().Equals("taza"))
                        units = "servido";
                    ...
                    if (ingredient.ToLower().Equals("lechuga") && units.Replace("s", "").Equals("hoja"))
                        units = "Hoja,exterior";
                    ...
                    if (ingredient.Equals("perejil") && (units.Equals("Tallo") || units.Equals("atado")))
                        units = "ramitas";
    ...
    private void QuantityFracTransform(string quantity, string recipeId, string measurisResult)
    {
        if (quantity.Contains("/"))
        {
            if (quantity.Length == 5)
            {
                ...
                InsertRecipeMeasuris(recipeId, measurisResult, intergerPart.ToString(), numerador.ToString(),
            ...
            else
            {
                ...
                InsertRecipeMeasuris(recipeId, measurisResult, 0.ToString(), numerador.ToString(),
                    denominador.ToString());
            }
        }
        else
        {
            if (quantity.Contains(" "))
            {
                ...
                if (numDem.Count > 0)
                {
                    InsertRecipeMeasuris(recipeId, measurisResult.ToString(), intergerPart.ToString(),
                        numDem[0].ToString(), numDem[1].ToString());
                }
            }
            else
            {
                if (quantity.Contains("."))
                {
                    InsertRecipeMeasuris(recipeId, measurisResult.ToString(), quantity.Replace(".5", ""), 0.ToString(),
                        0.ToString());
                }
                else
                {
    ...
    public void InsertRecipeMeasuris(string recipeId, string measurisResult, string quantity, string numerator,
        string denominator)
    {
        string insertRecipeMeasuris =
            $"INSERT INTO nutrifoods.recipe_measure (recipe_id, ingredient_measure_id, integer_part, numerator, denominator) " +
